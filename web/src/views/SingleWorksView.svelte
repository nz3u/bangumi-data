...
<div class="rise grid grid-cols-[minmax(0,1fr)] gap-4">
  ...
  {:else if data}
    ...
    <div class="card p-4">
      <div class="flex flex-wrap items-center justify-center gap-4">
        <div class="flex items-center gap-3">
          ...
          />
          <div>
            <button type="button" class="cursor-pointer font-semibold text-sakura-600 hover:underline dark:text-sakura-400" onclick={() => openDetail('person', data.person.id, data.person)}>{data.person.name}</button>
    ...
    {#if data.items.length === 0}
      <div class="card py-8 text-center text-sm text-neutral-500">未找到该人物参与的作品</div>
    {:else}
      <div class="relative">
            ...
            >
              ...
              <div class="flex min-h-0 flex-1 flex-col items-stretch gap-1 overflow-y-auto pb-0.5">
                {#each posTagsShown as t (t.key)}
                  <button
                    type="button"
                    class={`flex w-full shrink-0 items-baseline justify-between gap-1 rounded-full px-1.5 py-0.5 text-[11px] ${selPos.includes(t.key) ? 'bg-sakura-600 font-medium text-white hover:bg-sakura-500' : 'border border-neutral-300 bg-white text-neutral-600 hover:border-sakura-400 hover:text-sakura-600 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:border-sakura-500 dark:hover:text-sakura-400'}`}
        ...
        <div class="grid min-w-0 gap-3">
          ...
          {#if filtered && filtered.groups.length === 0}
            <div class="card py-8 text-center text-sm text-neutral-500">{selPos.length > 0 ? '所选职位下未找到作品' : '无匹配内容'}</div>
          {:else if filtered}
            <!-- 按职务分组，隔行排列 -->
