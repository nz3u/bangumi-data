                  ...
                  {#each g.works as w, i (w.id)}
                    <div class="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 px-4 py-2 ">
                      ...
                      <span class="ml-auto flex min-w-0 max-w-full items-center gap-x-2 text-xs text-neutral-500 dark:text-neutral-400">
                        <span class="min-w-0 max-w-full truncate leading-7"><b class="font-medium text-neutral-600 dark:text-neutral-300">{data.person_a.name}:</b> <Highlight text={roleText(w.roles_a) || '—'} q={filter} scope="filter" /></span>
                        <span class="min-w-0 max-w-full truncate leading-7"><b class="font-medium text-neutral-600 dark:text-neutral-300">{data.person_b.name}:</b> <Highlight text={roleText(w.roles_b) || '—'} q={filter} scope="filter" /></span>
                      </span>
                    </div>
                  {/each}
                </div>
