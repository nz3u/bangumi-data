        ...
        <table class="tbl">
          ...
          <tbody>
            {#each result.items as it (it.id)}
              <tr>
                <td class="text-neutral-500"><a href={`https://bgm.tv/character/${it.id}`} target="_blank" rel="noreferrer" class="hover:underline">{it.id}</a></td>
                <td class="max-w-64 truncate"><a href={`https://bgm.tv/character/${it.id}`} target="_blank" rel="noreferrer" class="text-sky-600 hover:underline dark:text-sky-400">{it.name}</a></td>
