...
import { externalUrl, isSearchHighlight } from '../lib/settings.svelte.js'
          ...
          <tbody>
            {#each result.items as it (it.id)}
              <tr class="cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800/50" onclick={() => openDetail('character', it.id, it)}>
                ...
                <td class="max-w-64 truncate"><a href={externalUrl('character', it.id)} target="_blank" rel="noreferrer" class="text-sky-600 hover:underline dark:text-sky-400" onclick={(e) => e.stopPropagation()}>{#if isSearchHighlight()}<Highlight text={it.name} q={f.q} />{:else}{it.name}{/if}</a></td>
                <td class="max-w-52 truncate">{#if isSearchHighlight()}<Highlight text={it.name_cn} q={f.q} />{:else}{it.name_cn}{/if}{#if !it.name_cn}—{/if}</td>
                <td>{it.role_name}</td>
