          ...
          <tbody>
            {#each result.items as it (it.id)}
              <tr class="cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800/50" onclick={() => openDetail('person', it.id, it)}>
                ...
                <td class="max-w-64 truncate"><a href={externalUrl('person', it.id)} target="_blank" rel="noreferrer" class="text-sky-600 hover:underline dark:text-sky-400" onclick={(e) => e.stopPropagation()}>{#if isSearchHighlight()}<Highlight text={it.name} q={f.q} />{:else}{it.name}{/if}</a></td>
